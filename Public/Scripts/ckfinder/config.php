...
function CheckAuthentication()
{
	...
	session_start();
	return isset($_SESSION['ckfinderAuthorized']) && $_SESSION['ckfinderAuthorized'];
}
...
function substrAfterFirstDelimiter($string,$delimiter)
{
...
//var_dump('http://'.$_SERVER['SERVER_NAME'].'/Public/Files/__Uploads/');die();

$baseUrl = '/Public/Files/__Uploads/';

// Check subfolder
$dom = new DOMDocument();
$dom->loadXML(file_get_contents("../../../../../../Sls/Configs/Site/site.xml"));
$xpath = new DOMXPath($dom);
$results = $xpath->query("//configs/domainName/domain[@default='1']");
$domain = $results->item(0)->nodeValue;
if (contains($domain,"/"))
	$baseUrl = "/".substrAfterFirstDelimiter($domain,"/").$baseUrl;
...
$config['ResourceType'][] = Array(
		'name' => '__Deprecated',
		'url' => $baseUrl . '__Deprecated',
		'directory' => $baseDir . '__Deprecated',
		'maxSize' => 0,
		'allowedExtensions' => '',
		'deniedExtensions' => '');
...
$config['ResourceType'][] = Array(
		...
		'maxSize' => 0,
		'allowedExtensions' => '3gp,7z,ai,aif,asf,avi,bat,bin,bmp,csv,divx,dll,doc,docx,eml,eps,exe,fla,flv,gif,gz,gzip,html,htm,indd,iso,jar,jpeg,jpe,jpg,js,lnk,log,m4a,mdb,mid,mov,mp2,mp3,mp4,mpc,mpeg,mpg,ods,odt,ogg,pdf,png,ppt,pptx,ps,psd,pst,pub,pxd,qt,ram,rar,rm,rmi,rmvb,rtf,sdc,sitd,swf,sxc,sxw,tar,tgz,tif,tiff,ttf,txt,vob,vsd,wav,wma,wmv,xls,xlsx,xml,zip',
		'deniedExtensions' => '');
